...
def test_independent_holdout_validation_audit() -> None:
    ...
    assert report["summary"]["passing_points"] == 46
    assert report["summary"]["failing_points"] == 2
    assert report["summary"]["direction_matches"] == 36
    assert report["summary"]["max_relative_error"] < 0.30
    assert report["summary"]["parameters_refit"] is False
    assert report["summary"]["independent_holdout_validation_complete"] is False
    assert report["summary"]["independent_holdout_experiment_complete"]
    assert all(
        point["figure"] == 20
        and point["sequence_length"] == 4096
        and point["series"].endswith(":attention")
        for point in report["failure_points"]
    )
