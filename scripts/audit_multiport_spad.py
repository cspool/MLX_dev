#!/usr/bin/env python3
"""Audit H69's diagram-derived multi-port scratchpad mechanism."""
...
def build_audit(config: dict[str, Any]) -> dict[str, Any]:
    ...
    compile_path = output_root / "multiport-compile-manifest.json"
    run_path = output_root / "multiport-run-manifest.json"
    ...
    fixed_slowdowns: dict[str, dict[str, float]] = {}
    for key, record in compiler["records"].items():
        ...
        fixed_cycles = h64["cycles"][sequence][hardware]
        fixed_slowdowns.setdefault(sequence, {})[hardware] = summary["cycles"] / fixed_cycles
        ...
        checks = {
            ...
            "ports": adapter["ports"] == record["ports"]
            == parent["routing"]["mesh_width"],
            "axis": adapter["axis"] == record["axis"] == "x",
    ...
    compat_summary_path = output_root / "compat/bsmm-64.json"
    compat_adapter_path = output_root / "compat/bsmm-64-adapter.json"
    h66_summary_path = PROJECT_ROOT / "artifacts/environment/h66/runs/bsmm-64-first.json"
    h66_adapter_path = (
        PROJECT_ROOT / "artifacts/environment/h66/runs/bsmm-64-first-adapter.json"
    )
    compatibility = {
        "summary_file": qualify(compat_summary_path)["pass"],
        "adapter_file": qualify(compat_adapter_path)["pass"],
        "summary_exact": sha256_file(compat_summary_path) == sha256_file(h66_summary_path),
        "adapter_exact": sha256_file(compat_adapter_path) == sha256_file(h66_adapter_path),
    }
    ...
    source_text = "\n".join(
        ...
        for name in ("transformer", "runner", "adapter_hh", "adapter_cc")
    )
    integrity_checks = {
        ...
        "one_port_compatibility": all(compatibility.values()),
    ...
    return {
        ...
        "one_port_compatibility": compatibility,
        "cycles": run["cycles"],
        "speedups": run["speedups"],
        "fixed_to_multiport_slowdowns": fixed_slowdowns,
        "monotonic": monotonic,
