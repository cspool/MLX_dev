#!/usr/bin/env python3
"""Run H69 configs twice through diagram-derived column scratchpad ports."""
...
BUILD_ROOT = PROJECT_ROOT / "build/multiport-spad"
...
def run_one(
    task: tuple[str, str, int, str, Path, Path, Path, Path]
) -> dict[str, Any]:
    key, replay, ports, axis, driver, config_path, summary_path, adapter_path = task
    subprocess.run(
        [
            str(driver),
            "--config",
...
def main() -> int:
    ...
    compiler = json.loads(
        (output_root / "multiport-compile-manifest.json").read_text(encoding="utf-8")
    )
    driver = build_driver()
    run_root = output_root / "runs"
    run_root.mkdir(parents=True, exist_ok=True)
    tasks: list[tuple[str, str, int, str, Path, Path, Path, Path]] = []
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            print(
                f"[multiport-spad] {item['key']} {item['replay']} "
    ...
    path = output_root / "multiport-run-manifest.json"
