#!/usr/bin/env python3
"""Compile all 192 H114 coupled full-mesh fold configurations."""
...
def main() -> int:
    ...
    scales = [*config["scales"]["fit"], *config["scales"]["holdout"]]
    outputs = {}
    ...
    path = output_root / "coupled-full-mesh-compile-manifest.json"
    path.write_text(
        json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    passed = (
        len(outputs) == int(config["execution"]["required_configs"])
        and all(
