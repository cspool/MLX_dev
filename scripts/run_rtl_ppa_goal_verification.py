#!/usr/bin/env python3
"""Run fresh repository verification for the H204 RTL/PPA certificate."""
...
def main() -> int:
    ...
    ruff_stdout = output / "ruff-stdout.log"
    ruff_stderr = output / "ruff-stderr.log"
    ...
    environment["PYTHONPATH"] = (
        ...
        else f"{project_paths}{os.pathsep}{old_pythonpath}"
    )
    pytest, pytest_seconds = execute(pytest_command, environment=environment)
    pytest_stdout = output / "pytest-stdout.log"
    pytest_stderr = output / "pytest-stderr.log"
