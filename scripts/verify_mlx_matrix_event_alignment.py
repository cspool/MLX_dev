...
def main():
    ...
    jobs=[p for p in (tests/"pytest").rglob("job.json") if not any(a.is_symlink() for a in p.parents)
          and json.loads(p.read_text()).get("schema")==f"mlx_{args.family}_window_job_v1"]
    if len(jobs)!=(48 if args.family=="vector" else 22):raise ValueError("native window comparison scope differs")
    ...
    for i,job_file in enumerate(sorted(jobs)):
        ...
        lowered=vector_events(job) if args.family=="vector" else matrix_events(job)
