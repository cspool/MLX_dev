#!/usr/bin/env python3
"""Run H98 Figure 19 source paths twice through four DSAGEN SRAM ports."""
...
BUILD_ROOT = PROJECT_ROOT / "build/fig19-source-paths"
...
def main() -> int:
    ...
    compiler = json.loads(
        (output_root / "fig19-source-paths-compile-manifest.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=12) as executor:
        ...
        for completed, future in enumerate(
            ...
            if completed % 16 == 0 or completed == len(tasks):
                print(
                    f"[fig19-source] {completed}/{len(tasks)} "
    ...
    path = output_root / "fig19-source-paths-run-manifest.json"
    path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
    return 0 if len(records) == 48 and all(checks.values()) else 1
