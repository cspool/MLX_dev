#!/usr/bin/env python3
"""Audit H158 same-input scaled dot-product Attention execution."""
...
from scripts.compile_attention_functional import output_address
...
def numpy_reference(
    ...
    q = np.asarray(parent["actual_outputs"], dtype=np.float64).reshape(2, 2)
    k = np.asarray(contract["k"], dtype=np.float64)
    v = np.asarray(contract["v"], dtype=np.float64)
    scores = q @ k.T / math.sqrt(float(contract["head_dimension"]))
...
def xfer_wiring_check(document: dict[str, Any], config: dict[str, Any]) -> bool:
    ...
    qk_blocks = [block for block in document["blocks"] if block["stage"] == "qk"]
    softmax_blocks = [
        block for block in document["blocks"] if block["stage"] == "softmax"
    ]
    sv_blocks = [block for block in document["blocks"] if block["stage"] == "sv"]
    if len(qk_blocks) != 4 or len(softmax_blocks) != 2 or len(sv_blocks) != 4:
        return False
    score_events = set()
    for block in qk_blocks:
        ...
        item = xfers[0]
        if not (
            item["destination"] == placement["softmax"][row]
            and int(item["destination_tag"]) == 2
            and int(item["destination_register"]) == column
        ):
            return False
    ...
    for block in softmax_blocks:
        ...
        for item in xfers:
            identifier = item["id"]
            column = int(identifier.split("_c", 1)[1].split("_", 1)[0])
            dimension = int(identifier.rsplit("_d", 1)[1])
            expected_pe = placement["sv"][row * 2 + dimension]
            if not (
                item["destination"] == expected_pe
    ...
    return (
        len(probability_events) == 8
        and probability_events == {event for block in sv_blocks for event in block["wait_events"]}
    )
...
def build_audit(config: dict[str, Any]) -> dict[str, Any]:
    ...
    compile_path = output_root / "attention-functional-compile-manifest.json"
    run_path = output_root / "attention-functional-run-manifest.json"
    ...
    compile_checks = {
        "experiment": compiler["experiment_id"] == "H158",
    ...
    contract = config["operator_contract"]
    contract_checks = {
        "family": enabled_document["metadata"]["operator_family"] == "attention",
        "shape": int(contract["sequence_length"]) == 2
        and int(contract["head_dimension"]) == 2
        and int(contract["value_dimension"]) == 2,
        "q_parent": enabled_document["metadata"]["q_parent_path"]
        == config["frozen_inputs"]["fft_cmp_functional"]["path"]
        and [value for row in enabled_document["metadata"]["q_values"] for value in row]
        == parents["fft_cmp_functional"]["actual_outputs"],
        ...
        "memory_seeds": len(enabled_document["functional_execution"]["memory"]) == 12,
        "constant_seeds": len(enabled_document["functional_execution"]["registers"]) == 2
        and all(
    ...
    run_checks = {
        "experiment": run["experiment_id"] == "H158",
    ...
    reference = numpy_reference(config, parents["fft_cmp_functional"])
    ...
    score_checks = []
    actual_scores = []
    for row, pe in enumerate(contract["placement"]["softmax"]):
        values = []
        for column in range(2):
            actual = register_value(registers, pe=pe, tag=2, reg=column)
            values.append(actual)
            score_checks.append(
                actual is not None
                and math.isclose(
                    actual, reference["scores"][row][column], rel_tol=0.0, abs_tol=1e-12
                )
            )
        actual_scores.append(values)
    probability_checks = []
    actual_probabilities = []
    for row in range(2):
        row_values = []
        for dimension in range(2):
            pe = contract["placement"]["sv"][row * 2 + dimension]
            values = []
            for column in range(2):
                actual = register_value(registers, pe=pe, tag=3, reg=column)
                values.append(actual)
                probability_checks.append(
                    actual is not None
                    and math.isclose(
                        actual,
                        reference["probabilities"][row][column],
                        rel_tol=0.0,
                        abs_tol=1e-12,
                    )
                )
            row_values.append(values)
        actual_probabilities.append(row_values)
    intermediate_checks = {
        "scores": len(score_checks) == 4 and all(score_checks),
        "probabilities": len(probability_checks) == 8 and all(probability_checks),
    ...
    transfer_route_checks = {
        ...
        == {output_address(row, dimension) for row in range(2) for dimension in range(2)},
    }
    timing_checks = {
    ...
    performance_speedups = parents["attention_performance"]["summary"]["speedups"]
    performance_context = {
        "qualified_rows": len(performance_speedups),
        "minimum_speedup": min(float(value) for value in performance_speedups.values()),
        "maximum_speedup": max(float(value) for value in performance_speedups.values()),
        "all_clear_gain": all(float(value) >= 1.2 for value in performance_speedups.values()),
    }
    parent_semantic_checks = {
        "h157_chain": parents["fft_cmp_functional"]["summary"][
            "completed_operator_payloads"
        ]
        == ["bsmm", "fft_cmp"],
        "h135_attention": performance_context["qualified_rows"] == 2
    ...
    integrity_checks = {
        ...
        "intermediates_evaluated": len(score_checks) == 4
        and len(probability_checks) == 8,
    ...
    return {
        ...
        "paper_reproduction_claim": "attention_functional_plus_existing_composition_gain_only",
        "functional_claim": "same_input_qk_stable_softmax_sv_on_timed_spatial_schedule",
        ...
        "summary": {
            ...
            "attention_functional_complete": supported,
            "completed_operator_payloads": ["bsmm", "fft_cmp", "attention"]
            if supported
            else ["bsmm", "fft_cmp"],
            "operator_payload_coverage": 3 if supported else 2,
            "required_operator_payloads": 6,
            "existing_attention_speedup_minimum": performance_context["minimum_speedup"],
            "existing_attention_speedup_maximum": performance_context["maximum_speedup"],
