#!/usr/bin/env python3
"""Build and execute H82 grouped-attention fixtures twice."""
...
BUILD_ROOT = PROJECT_ROOT / "build/grouped-attention-cdc"
...
def execute(driver: Path, config_path: Path, summary_path: Path) -> dict[str, Any]:
    subprocess.run(
        [
            str(driver),
            "--config",
...
def run_one(task: tuple[str, str, Path, Path, Path]) -> dict[str, Any]:
    ...
    result = execute(driver, config_path, summary_path)
    return {"key": key, "replay": replay, **result}
...
def main() -> int:
    ...
    compiler = json.loads(
        (output_root / "grouped-attention-compile-manifest.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    legacy_config = PROJECT_ROOT / "artifacts/environment/h80/configs/N256-q4.json"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            print(
                f"[grouped-attention] {item['key']} {item['replay']} "
    ...
    path = output_root / "grouped-attention-run-manifest.json"
