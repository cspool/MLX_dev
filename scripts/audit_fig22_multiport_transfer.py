#!/usr/bin/env python3
"""Join frozen H120 primary utilizations to all 64 Figure 22 targets."""
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs/simulators/fig22_multiport_transfer_v1.yaml"
...
def build_audit(config: dict[str, Any]) -> dict[str, Any]:
    ...
    parent_checks = {
        ...
        "h120_target_free": all(h120["target_free_checks"].values())
        and h120["paper_performance_targets_consumed"] is False,
    }
    h120_run_path = PROJECT_ROOT / h120["run_manifest"]["path"]
    h120_run_file = qualify(h120_run_path, h120["run_manifest"])
    h120_run = json.loads(h120_run_path.read_text())
    parent_checks["h120_regressions"] = (
        h120_run_file["pass"] and h120_run["checks"]["regressions"] is True
    )
    ...
    for operator, panel in config["mapping"].items():
        ...
        for index, size in enumerate(sizes):
            key = f"{operator}-{size}"
            measurement = h120["measurements"][key]
            mapping_checks[key] = (
                ...
                points.append(
                    {
                        ...
                        "prediction_provenance": f"H120.measurements.{key}.{prediction_field}.{resource}",
    ...
    forbidden_tokens = (
        ...
        "pointwise" + "_oracle",
        "correction" + "_factor",
        "diagnostic" + "_overlay_utilization",
        "single_port" + "_prediction",
    )
    source_checks = {
        "no_selection_or_fit": not any(
        ...
        "only_h120_prediction_parent": set(config["frozen_inputs"]) == {"h120", "h60"},
    }
    ...
    acceptance_gates = [
        ...
        and all(point["prediction_provenance"].startswith("H120.") for point in points),
        all(point["target_provenance"].startswith("H60.") for point in points),
        all(finite_checks.values()),
        all_points_pass,
        global_summary["points"] == 64
        and all(item["points"] == 16 for item in by_resource.values())
        and all(item["points"] == 32 for item in by_operator.values()),
        parent_checks["h120_target_free"] and parent_checks["h120_regressions"],
        ...
        and int(config["acceptance"]["required_passing_points"])
        == int(config["mapping"]["required_points"]),
    ]
    integrity_checks = {
        "frozen": all(item["pass"] for item in frozen.values()),
        "parents": all(parent_checks.values()),
        "h120_run": h120_run_file["pass"],
    ...
    return {
        ...
        "h120_run_manifest": h120_run_file,
