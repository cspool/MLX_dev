#!/usr/bin/env python3
"""Execute H129 q64/q128 configs across optimized/ASan/UBSan modes."""
...
def main() -> int:
    ...
    compiled = json.loads(
        (output_root / "fig19-coupled-steady-state-compile-manifest.json").read_text()
    )
    build_root = PROJECT_ROOT / "build/fig19-coupled-steady-state"
    drivers: dict[str, Path] = {}
    for mode in MODE_FLAGS:
        driver = build_root / f"fig19-coupled-steady-{mode}"
        ...
        for mode, replay in specifications:
            ...
            tasks.append(
                (
                    ...
                    summary,
                    int(h128_config["execution"]["max_cycles"]),
                )
            )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for completed, future in enumerate(
            ...
            if completed % 10 == 0 or completed == len(tasks):
                ...
                print(
                    f"[H129] {completed}/{len(tasks)} {record['key']} "
    ...
    path = output_root / "fig19-coupled-steady-state-run-manifest.json"
