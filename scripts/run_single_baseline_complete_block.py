#!/usr/bin/env python3
"""Run all H170 paired complete-block configs across three C++ builds."""
...
def main() -> int:
    ...
    compiler = json.loads(
        (output_root / "single-baseline-complete-compile-manifest.json").read_text()
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for completed, future in enumerate(
            ...
            if completed % 8 == 0 or completed == len(tasks):
                ...
                print(
                    f"[H170] {completed}/{len(tasks)} {item['name']} "
    ...
    path = output_root / "single-baseline-complete-run-manifest.json"
