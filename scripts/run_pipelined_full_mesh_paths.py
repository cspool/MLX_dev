#!/usr/bin/env python3
"""Run H110 corrected full-mesh configs twice through H66 SRAM."""
...
BUILD_ROOT = PROJECT_ROOT / "build/pipelined-full-mesh-paths"
...
def run_one(task: tuple[str, str, Path, Path, Path, Path, int, str]) -> dict[str, Any]:
    ...
    if result.returncode != 0:
        raise RuntimeError(f"{key}/{replay}: {result.stderr}")
    return {
        "key": key,
        "replay": replay,
...
def main() -> int:
    ...
    compiled = json.loads(
        (output_root / "pipelined-full-mesh-compile-manifest.json").read_text()
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for completed, future in enumerate(
            ...
            if completed % 32 == 0 or completed == len(tasks):
                print(
                    f"[H110] {completed}/{len(tasks)} {item['key']} "
    ...
    path = output_root / "pipelined-full-mesh-run-manifest.json"
    path.write_text(
        json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    return 0 if (
        len(records) == int(config["required"]["configs"])
