#!/usr/bin/env python3
"""Run H93 Attention configs twice through four DSAGEN SRAM ports."""
...
BUILD_ROOT = PROJECT_ROOT / "build/fig21-attention-timing"
...
def main() -> int:
    ...
    compiler = json.loads(
        (output_root / "fig21-attention-compile-manifest.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for completed, future in enumerate(
            ...
            if completed % 10 == 0 or completed == len(tasks):
                print(
                    f"[fig21-attention] {completed}/{len(tasks)} "
    ...
    path = output_root / "fig21-attention-run-manifest.json"
