#!/usr/bin/env python3
"""Derive an AGX Orin GPGPU-Sim proxy from the tested SM86 RTX3070 model."""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "artifacts/environment/h54/config"
...
def derive_config(text: str) -> tuple[str, dict[str, int]]:
    replacements = {
        "clusters": ("-gpgpu_n_clusters 46", "-gpgpu_n_clusters 16"),
        "clocks": (
...
def main() -> int:
    ...
    manifest = {
        ...
        "memory_partitions_unchanged": derived.count("-gpgpu_n_mem 16") == 1,
    }
    manifest_path = output_dir / "orin-config-manifest.json"
