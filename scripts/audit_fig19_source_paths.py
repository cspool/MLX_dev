#!/usr/bin/env python3
"""Audit H98 source-integrated Figure 19 paths."""
...
def build_audit(config: dict[str, Any]) -> dict[str, Any]:
    ...
    compile_path = output_root / "fig19-source-paths-compile-manifest.json"
    run_path = output_root / "fig19-source-paths-run-manifest.json"
    ...
    for run_key, item in compiler["outputs"].items():
        ...
        if path_key.endswith("-fft2d"):
            n = int(path_key.split("-", maxsplit=1)[0].removeprefix("N"))
    ...
    for path_key, contract in compiler["path_contracts"].items():
        model = fit_affine(
            fit_scales[0], measurements[f"{path_key}-q{fit_scales[0]}"]["cycles"],
            fit_scales[1], measurements[f"{path_key}-q{fit_scales[1]}"]["cycles"],
        )
        holdouts = []
    ...
    integrity_checks = {
        ...
        "compile_manifest": compile_file["pass"] and len(compiler["outputs"]) == 48,
        "run_manifest": run_file["pass"] and len(run["records"]) == 48,
        ...
        "counts": summary["path_count"] == 12
        and summary["run_count"] == 48
        and summary["total_holdouts"] == 24,
    }
