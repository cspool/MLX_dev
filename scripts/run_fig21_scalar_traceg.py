#!/usr/bin/env python3
"""Replay H147 scalar-service traces with Accel-Sim Xavier timing."""
...
def run_one(
    *,
    ...
    measurement = {
        ...
        "warp_instructions": record["warp_instructions"],
        "scalar_operations": record["scalar_operations"],
...
def main() -> int:
    ...
    compile_manifest = json.loads((output_root / "scalar-traceg-compile-manifest.json").read_text())
    records = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for completed, future in enumerate(concurrent.futures.as_completed(future_map), start=1):
            ...
            print(
                f"[H147] {completed}/{len(future_map)} {record['key']} "
    ...
    path = output_root / "scalar-traceg-run-manifest.json"
    path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
    return 0 if len(records) == 12 and all(item["pass"] for item in records.values()) else 1
