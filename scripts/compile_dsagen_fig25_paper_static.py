#!/usr/bin/env python3
"""Transform H50 arithmetic configs to H53 paper-static PE semantics."""
...
DEFAULT_PARENT = PROJECT_ROOT / "artifacts/environment/h50"
DEFAULT_OUTPUT = PROJECT_ROOT / "artifacts/environment/h53"
...
def transform(parent: dict[str, Any]) -> dict[str, Any]:
...
def main() -> int:
    ...
    outputs = []
    for parent_path in sorted(parent_dir.glob("*--*.json")):
        output_path = output_dir / parent_path.name
        parent = json.loads(parent_path.read_text(encoding="utf-8"))
        output_path.write_text(canonical_json(transform(parent)), encoding="utf-8")
        outputs.append(
            {
                "name": parent_path.stem,
                "parent": digest(parent_path),
                "output": digest(output_path),
                "operator": parent["metadata"]["operator"]["name"],
                "case": parent["metadata"]["case"]["name"],
            }
        )
    if len(outputs) != 24:
        raise ValueError(f"expected 24 H50 parent configs, found {len(outputs)}")
    manifest = {
        "schema_version": 1,
        "experiment_id": "H53",
    ...
    (output_dir / "fig25-paper-static-compile-manifest.json").write_text(
