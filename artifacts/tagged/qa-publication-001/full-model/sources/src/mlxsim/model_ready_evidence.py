...
from .model_result_contract import QA_CONTRACT, verify_qa_result
...
def verify_ready_execution(program,result,options):
    ...
    for spec,actual in zip(program["outputs"],outputs,strict=True):
        if program.get("output_contract")==QA_CONTRACT:
            qa_count,qa_bytes=verify_qa_result(program,spec,actual);readback+=qa_count;readback_bytes+=qa_bytes
            continue
