...
from mlxsim.model_result_contract import QA_CONTRACT, compile_qa_outputs, result_roles, validate_result_contract
...
def compile_inventory(inventory, *, matrix_backend="blas", schedule_options=None, vector_backend="functional", vector_schedule_options=None, memory_backend="functional", memory_schedule_options=None, control_backend="functional", control_schedule_options=None):
    ...
    if any(event["operator"] in {LAYER_NORM,GELU} for event in inventory.get("operations", [])):
        ...
        program["schema"]="mlx_tensor_semantics_v4" if program.get("output_contract")==QA_CONTRACT else "mlx_tensor_semantics_v3"
        program["value_contract"]="source_groups_v1";program["source_groups"]=groups
        for group in groups:
    ...
    if len(checks) != len(inventory["reference_checks"]):
        raise ValueError("duplicate reference forward identity")
    qa = any("outputs" in check for check in checks.values())
    ...
    if qa:
        if outputs:
            raise ValueError("QA and generation output protocols cannot be mixed")
        outputs = compile_qa_outputs(inventory, current, assets, nodes)
    ...
    for output in outputs:
        for role in result_roles({"output_contract": QA_CONTRACT} if qa else {}):
            last_use[output[role]] = len(nodes)
    release_at = {}
    for position, node in enumerate(nodes):
    ...
    program = {
        ...
        "input_contract": ({"task": "extractive_qa", "metadata": "explicit_tokenizer_input_assets",
                            "forward_ids": list(checks)} if qa else inventory["input"]),
    ...
    if qa:
        program["schema"] = "mlx_tensor_semantics_v4"
        program["output_contract"] = QA_CONTRACT
