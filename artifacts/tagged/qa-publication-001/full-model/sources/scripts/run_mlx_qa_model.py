...
def audit_outputs(program, inventory, native):
    ...
    for n in program["nodes"]:
        ...
        require(e.get("entry") == "tensor_model::" + n["kind"] and e.get("forward_id") == n["forward_id"], "BERT event/backend identity differs")
...
def main():
    ...
    program, coverage = compile_inventory(inventory, matrix_backend="scheduled" if scheduled else "microcode",
        ...
        control_backend="scheduled" if scheduled else "rv64_leaf")
    ...
    passed = all(r["major_correctness_passed"] for r in comparison)
