...
struct Runner {
  ...
  Runner(const Json::Value &p,const Json::Value &options)
    ...
    require((p["schema"]=="mlx_tensor_semantics_v1"||p["schema"]=="mlx_tensor_semantics_v2"||p["schema"]=="mlx_tensor_semantics_v3"||p["schema"]=="mlx_tensor_semantics_v4")&&p["timing_mode"]=="unmodeled","unsupported ready graph program");
    validate_result_contract(p);
    ...
    for(const auto &out:p["outputs"]){require(out["forward_id"].isInt()&&forwards.insert(out["forward_id"].asInt()).second,"duplicate/invalid graph output forward");for(const auto &role:result_roles(p)){auto id=out[role].asString();require(values.count(id)||producer.count(id),"graph output is unbound");keep.insert(id);}}
  ...
  Json::Value run(const std::filesystem::path &directory){
    ...
    for(const auto &spec:program["outputs"]){
      if(qa_results(program)){outputs.append(execute_qa_result(program,spec,directory,[&](const std::string &id){return readback(values.at(id));}));continue;}
