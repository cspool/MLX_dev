...
struct Machine {
  ...
  Machine(const Json::Value &n,const Values &values,Stats &s):node(n),program(n["vector_program"]),stats(s) {
    ...
    check(program["input_dtypes"].isArray()&&program["input_dtypes"].size()==(node["kind"]=="add"||node["kind"]=="sub"||node["kind"]=="mul"||node["kind"]=="div"||node["kind"]=="maximum"?2u:1u),"vector operand arity mismatch");
...
bool supports(const std::string &kind){return kind=="div"||kind=="maximum"||kind=="exp"||kind=="sub"||kind=="add"||kind=="mul"||kind=="pow"||kind=="rsqrt"||kind=="silu"||kind=="cos"||kind=="sin"||kind=="neg"||kind=="mean"||kind=="softmax";}
