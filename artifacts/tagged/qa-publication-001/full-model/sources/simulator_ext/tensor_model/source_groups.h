...
inline void validate_source_groups(const Json::Value &program){
  ...
  require((program["schema"]=="mlx_tensor_semantics_v3"||program["schema"]=="mlx_tensor_semantics_v4")&&program["value_contract"]=="source_groups_v1"&&groups.isArray()&&!groups.empty(),"invalid source-group contract");
  unsigned cursor=0;
