...
int main(int argc,char **argv) {
  try {
    ...
    require((program["schema"]=="mlx_tensor_semantics_v1"||program["schema"]=="mlx_tensor_semantics_v2"||program["schema"]=="mlx_tensor_semantics_v3"||program["schema"]=="mlx_tensor_semantics_v4") && program["timing_mode"]=="unmodeled",
    ...
    for (const auto &spec:program["outputs"]) {
      if(qa_results(program)){results.append(execute_qa_result(program,spec,output,[&](const std::string &id){return values.at(id).materialize(values.at(id).type);}));continue;}
