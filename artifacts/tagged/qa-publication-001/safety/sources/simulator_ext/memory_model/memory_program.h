...
inline bool extended_kind(const std::string &kind){return kind=="advanced_index"||kind=="new_ones"||kind=="squeeze"||kind=="split";}
inline const char *profile(const std::string &kind){return kind=="split"?"mlx-memory-plan-v3":extended_kind(kind)?"mlx-memory-plan-v2":"mlx-memory-plan-v1";}
