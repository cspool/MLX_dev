...
def require_value_contract(program):
    ...
    expected = "mlx_tensor_semantics_v4" if "output_contract" in program else "mlx_tensor_semantics_v3" if grouped else "mlx_tensor_semantics_v2" if split else "mlx_tensor_semantics_v1"
    ...
    for output in program.get("outputs", []):
        if any(output.get(role) in owners for role in result_roles(program)):
