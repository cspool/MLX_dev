...
module MLXNativeRoCCBlackBox #(
    parameter BACKEND=2, XLEN=64, ADDR_BITS=40, TAG_BITS=6, CMD_BITS=5,
  ...
  import "DPI-C" function chandle mlx_system_create(input int unsigned address_bits);
  import "DPI-C" function void mlx_system_reset(input chandle model);
  import "DPI-C" function void mlx_system_destroy(input chandle model);
  import "DPI-C" function void mlx_system_tick(
  ...
  import "DPI-C" function void mlx_system_eval(
  ...
  initial begin
    if (BACKEND != 2 || XLEN != 64 || DATA_BITS != 64 || DATA_BYTES != 8)
      $fatal(1,"unsupported native model interface");
    model = mlx_system_create(ADDR_BITS);
    if (model == null) $fatal(1,"native model construction failed");
  end
  always @(posedge clock) begin
    if (reset) mlx_system_reset(model);
    else mlx_system_tick(model,cmd_valid,32'(cmd_funct),cmd_xd,32'(cmd_rd),cmd_rs1,cmd_rs2,
  ...
  always @* begin
    ...
    mlx_system_eval(model,epoch,reset,32'(cmd_funct),resp_ready,cr,rv,rd_full,data_full,db,
  ...
  final mlx_system_destroy(model);
endmodule
