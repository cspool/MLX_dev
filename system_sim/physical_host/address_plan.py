...
def iter_bindings(program,life):
    ...
    for node,event in zip(program["nodes"],life["events"],strict=True):
        ...
        if any(name not in live for name in references(node["args"])):raise ValueError("descriptor references a released SSA value")
        ...
        if node["id"] in live:raise ValueError("descriptor SSA was redefined")
        live.add(node["id"])
