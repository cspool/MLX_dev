...
namespace {
const char *kinds[]={"","embedding","where","cat","cast","cast_device","contiguous","reshape","transpose","slice","select","unsqueeze","expand","alias","dropout_inference"};
const char *selectors[]={"","linear","concat","indexed_rows","predicate_select"};
...
DecodedMemory decode_memory(const mlx_memory_wire &wire){
  require(wire.magic==MLX_MEMORY_WIRE_MAGIC&&wire.version==1&&wire.kind>=1&&wire.kind<=14&&wire.mode<=1&&wire.selector>=1&&wire.selector<=4,"memory wire header invalid");
  require(!(wire.flags&~UINT64_C(63))&&zero(wire.reserved,sizeof(wire.reserved))&&zero(wire.tail_reserved,sizeof(wire.tail_reserved)),"memory wire reserved fields invalid");
  ...
  require(wire.operand_count<=64&&wire.argument_count>0&&wire.argument_count<=256&&wire.word_count<=4&&wire.param_count<=16,"memory wire control capacity exceeded");
  ...
  p["profile"]="mlx-memory-plan-v1";p["kind"]=node["kind"];p["mode"]=result.view?"view":"transfer";p["selector"]=selectors[wire.selector];p["reason"]="wire_checked_layout";
  ...
  for(unsigned i=0;i<64;++i){
    ...
    if(arg.kind==MLX_HOST_TENSOR){
      ...
      require(arg.kind==MLX_HOST_SCALAR&&arg.root_id==0&&zero(&arg.tensor,sizeof(arg.tensor)),"memory scalar operand invalid");
  ...
  p["words"]=Json::Value(Json::arrayValue);for(unsigned i=0;i<4;++i){if(i<wire.word_count){require(wire.words[i]<=UINT32_MAX,"memory instruction word is not 32-bit");p["words"].append(Json::UInt(wire.words[i]));}else require(!wire.words[i],"unused memory instruction is nonzero");}
  ...
  if(wire.kind==MLX_MEMORY_EMBEDDING){require(args.size()==2,"memory embedding source count mismatch");count(3);require(wire.params[1]<=1&&wire.params[2]<=1,"memory embedding flags invalid");args.append(param(0));args.append(bool(wire.params[1]));args.append(bool(wire.params[2]));}
    ...
    switch(wire.kind){
      ...
      case MLX_MEMORY_RESHAPE:case MLX_MEMORY_EXPAND:{require(wire.param_count<=8,"memory requested shape rank exceeds bound");Json::Value shape(Json::arrayValue);for(unsigned i=0;i<wire.param_count;++i)shape.append(param(i));args.append(shape);break;}
      case MLX_MEMORY_TRANSPOSE:case MLX_MEMORY_SELECT:count(2);args.append(param(0));args.append(param(1));break;
      case MLX_MEMORY_UNSQUEEZE:count(1);args.append(param(0));break;
