// Simulation-only 64-bit checked-address wrapper; derived from pinned TestChipIP SimDRAM.v.
import "DPI-C" function void mlx_wide_memory_reset(input chandle channel);
import "DPI-C" function void mlx_wide_memory_destroy(input chandle channel);
import "DPI-C" function chandle mlx_wide_memory_init
(
  input longint unsigned mem_base,
  input longint unsigned mem_size,
  input longint unsigned word_size,
  input longint unsigned line_size
);

import "DPI-C" function void mlx_wide_memory_tick
(
  ...
  input  bit     ar_valid,
  output bit     ar_ready,
  input  longint unsigned ar_addr,
  input  int unsigned     ar_id,
  input  int unsigned     ar_size,
  input  int unsigned     ar_len,
  input  int unsigned     ar_burst,

  input  bit     aw_valid,
  output bit     aw_ready,
  input  longint unsigned aw_addr,
  input  int unsigned     aw_id,
  input  int unsigned     aw_size,
  input  int unsigned     aw_len,
  input  int unsigned     aw_burst,

  input  bit     w_valid,
  output bit     w_ready,
  input  int unsigned     w_strb,
  input  longint unsigned w_data,
  input  bit     w_last,

  output bit     r_valid,
  input  bit     r_ready,
  output int unsigned     r_id,
  output int unsigned     r_resp,
  output longint unsigned r_data,
  output bit     r_last,

  output bit     b_valid,
  input  bit     b_ready,
  output int unsigned     b_id,
  output int unsigned     b_resp
);

module MLXWideMemory #(
    parameter longint unsigned MEM_BASE=64'h80000000,
    parameter longint unsigned MEM_SIZE=1000 * 1000 * 1000,
  ...
  always @(posedge clock) begin
    if (reset) begin
      if (initialized) mlx_wide_memory_reset(channel);
      __ar_ready = 1'b0;
      __aw_ready = 1'b0;
      __w_ready  = 1'b0;
      __r_valid  = 1'b0;
      ...
      if (!initialized) begin
        channel = mlx_wide_memory_init(MEM_BASE, MEM_SIZE, WORD_SIZE, LINE_SIZE);
        initialized = 1'b1;
      end

      mlx_wide_memory_tick(
        channel,
  ...
  final if (initialized) mlx_wide_memory_destroy(channel);
endmodule
