...
class MLXClockedRoCCBlackBox(
  ...
  Seq("MLXClockedRoCC.sv").foreach(name => addResource(s"/vsrc/$name"))
}

class MLXClockedRoCC(opcodes: OpcodeSet, backend: Int)(implicit p: Parameters) extends LazyRoCC(opcodes) {
  val backendKind: Int = backend
  override lazy val module = new MLXClockedRoCCModuleImp(this)
}

class MLXClockedRoCCModuleImp(outer: MLXClockedRoCC)(implicit p: Parameters)
    extends LazyRoCCModuleImp(outer)
    with HasCoreParameters {
  private val box = Module(
    new MLXClockedRoCCBlackBox(
      outer.backendKind,
      xLen,
      coreMaxAddrBits,
      coreParams.dcacheReqTagBits,
      M_SZ,
...
class WithMLXClockedRoCC(backend: Int) extends Config((site, here, up) => {
  case BuildRoCC => up(BuildRoCC, site) ++ Seq(
    (p: Parameters) => {
      val mlx = LazyModule(new MLXClockedRoCC(OpcodeSet.custom0, backend)(p))
      mlx
    })
})

// The accelerator is the native C++ behavior model; Rocket remains Chipyard RTL.
class MLXClockedRocketConfig extends Config(
