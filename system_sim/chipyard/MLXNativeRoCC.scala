...
class MLXNativeRoCCBlackBox(
  ...
  Seq("MLXNativeRoCC.sv").foreach(name => addResource(s"/vsrc/$name"))
}

class MLXNativeRoCC(opcodes: OpcodeSet, backend: Int)(implicit p: Parameters) extends LazyRoCC(opcodes) {
  val backendKind: Int = backend
  override lazy val module = new MLXNativeRoCCModuleImp(this)
}

class MLXNativeRoCCModuleImp(outer: MLXNativeRoCC)(implicit p: Parameters)
    extends LazyRoCCModuleImp(outer)
    with HasCoreParameters {
  private val box = Module(
    new MLXNativeRoCCBlackBox(
      outer.backendKind,
      xLen,
      coreMaxAddrBits,
...
class WithMLXNativeRoCC(backend: Int) extends Config((site, here, up) => {
  case BuildRoCC => up(BuildRoCC, site) ++ Seq(
    (p: Parameters) => {
      val mlx = LazyModule(new MLXNativeRoCC(OpcodeSet.custom0, backend)(p))
      mlx
    })
})

// The accelerator is the native C++ behavior model; Rocket remains Chipyard RTL.
class MLXNativeRocketConfig extends Config(
  new WithMLXNativeRoCC(2) ++
