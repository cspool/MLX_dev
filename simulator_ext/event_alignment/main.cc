...
int main(int argc,char **argv){
  try{
    ...
    shared_array::Resources array(h);Assets loader;std::vector<std::unique_ptr<Window>> windows;
    for(unsigned source=0;source<job["windows"].size();++source){const auto &spec=job["windows"][source];auto w=std::make_unique<Window>();
      if(spec["schema"]=="mlx_matrix_window_job_v1"){
        w->a=loader.load(spec["a"]);w->b=loader.load(spec["b"]);bool bias=spec.isMember("bias");if(bias)w->bias=loader.load(spec["bias"]);
        ...
        w->matrix=std::make_unique<matrix_schedule::Simulator>(spec["program"],w->a,w->b,bias?&w->bias:nullptr,spec.get("transposed_b",true).asBool(),0,0,m,n,k,w->output,0,o,nullptr,&array,source);
      }else{
        require(spec["schema"]=="mlx_vector_window_job_v1","unsupported native group family");for(const auto &name:spec["assets"].getMemberNames())w->values.emplace(name,loader.load(spec["assets"][name]));
        const auto &node=spec["node"];w->output=Tensor::allocate(dtype(node["output"]["dtype"].asString()),shape(node["output"]["shape"]));
        auto o=vector_schedule::Options::parse(spec["options"]);require(o.overlap,"native group must preserve concurrency");w->vector=std::make_unique<vector_schedule::Simulator>(node,w->values,w->output,o,nullptr,&array,source);
      }
      windows.push_back(std::move(w));
    }
    uint64_t cycles=0;for(;;){bool work=false;for(const auto &w:windows)work|=!w->done();if(!work)break;
      require(cycles<job.get("max_cycles",Json::UInt64(10000000)).asUInt64(),"native group cycle budget exceeded");array.begin_cycle(cycles);for(auto &w:windows)if(!w->done())w->tick();array.end_cycle();++cycles;
    }
