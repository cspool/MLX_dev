...
bool Hardware::operator==(const Hardware &o)const{
  return std::tie(rows,columns,contexts,spm_period,writeback_period,compute_ii,sfu_ii,dma_request_period,dma_response_period,multiply_latency,add_latency,convert_latency,spm_latency,exp_latency,div_latency,sqrt_latency)==
         std::tie(o.rows,o.columns,o.contexts,o.spm_period,o.writeback_period,o.compute_ii,o.sfu_ii,o.dma_request_period,o.dma_response_period,o.multiply_latency,o.add_latency,o.convert_latency,o.spm_latency,o.exp_latency,o.div_latency,o.sqrt_latency);
}
...
void Resources::begin_cycle(uint64_t cycle){
  ...
  open=true;current_cycle=cycle;current_client=0;last_priority.reset();issued.fill(false);written.fill(false);spm_claimed=false;
}
...
std::optional<Lease> Resources::admit(const Offer &proposal,uint64_t block){
  ...
  if(!code){templates[lease.pe].push_back(Template{next_template++,info.key,info.words,lease.rom_base,0});code=&templates[lease.pe].back();
    for(unsigned i=0;i<lease.rom_count;++i){check(!rom_owner[lease.pe][lease.rom_base+i],"non-atomic ROM allocation");rom_owner[lease.pe][lease.rom_base+i]=code->id;rom[lease.pe][lease.rom_base+i]=info.words[i];}
    template_words_loaded+=lease.rom_count;
  }
  ++code->refs;
...
uint32_t Resources::instruction(const Lease &lease,unsigned at)const{caller(lease.client);const auto &r=resident(lease);check(at<lease.rom_count&&rom_owner[lease.pe][lease.rom_base+at]==r.template_id,"instruction access outside owned template");return rom[lease.pe][lease.rom_base+at];}
...
bool Resources::issue_ready(unsigned pe)const{check(open&&pe<pes,"issue queried outside array edge");return !issued[pe];}
...
void Resources::claim_issue(const Lease &lease){caller(lease.client);resident(lease);check(issue_ready(lease.pe),"shared PE issue bandwidth exceeded");issued[lease.pe]=true;}
...
void Resources::claim_unit(Unit unit,const Lease &lease){
  caller(lease.client);resident(lease);check(unit_ready(unit,lease.pe),"shared service slot/II exceeded");auto &s=service(unit,lease.pe);
...
Json::Value Resources::snapshot()const{
  ...
  r["template_words_loaded"]=Json::UInt64(template_words_loaded);r["template_configuration_cycles_modeled"]=false;
