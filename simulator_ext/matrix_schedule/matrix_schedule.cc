...
struct Register { std::array<uint32_t,16> data{}; unsigned type=0; };
struct Context {
...
struct Simulator::Impl {
  ...
  std::optional<std::tuple<unsigned,unsigned>> admission_offer;
  ...
  Impl(const Json::Value &p,Tensor a,Tensor b,const Tensor *bias_arg,bool tb,uint64_t ab,uint64_t bb,
       uint64_t mm,uint64_t nn,uint64_t kk,Tensor out,uint64_t ob,Options o,model_io::MemoryPort *port)
    ...
    spm_owner.fill(-1);total_blocks=((m+1)/2)*((n+15)/16);
  ...
  Register &reg(Context &c,unsigned index) {return rf[c.pe][c.slot*6+index];}
  ...
  void recompute_control(){
    ...
    if(next_block<total_blocks){unsigned pe=next_block%pes;
      for(unsigned slot=0;slot<options.contexts&&!admission_offer;++slot)if(!contexts[pe*2+slot].active){
        for(unsigned start=0;start+arena_vectors<=128;++start){bool free=true;
          for(unsigned v=start;v<start+arena_vectors;++v)free&=spm_owner[v]<0;
          if(free){admission_offer=std::make_tuple(pe*2+slot,start);break;}
        }
      }
    }
  ...
  PendingDMA request(unsigned id) {
    ...
    if(r.write)std::memcpy(&r.data,spm.data()+r.spm,r.bytes);
    return r;
  }
  void finish_memory(unsigned id) {
    ...
    if(c.phase==Fill){c.phase=Body;c.pc=0;c.ki=0;event("wake",id);}
      ...
      for(unsigned v=c.arena;v<c.arena+arena_vectors;++v){check(spm_owner[v]==int(id),"SPM ownership corrupt at retirement");spm_owner[v]=-1;}
  ...
  Register evaluate(Context &c,const Instruction &i) {
    ...
    if(memory_op(i.op)){
      ...
      for(unsigned lane=0;lane<c.lanes;++lane){unsigned address=(i.op==LoadA16||i.op==LoadA32)?a_base+(i.row*64+c.ki)*bytes:(i.op==LoadB16||i.op==LoadB32)?(c.ki*16+lane)*bytes:bias_base+lane*bytes;std::memcpy(&result.data[lane],spm.data()+c.arena*64+address,bytes);}return result;
    }
  ...
  void complete_instruction(PendingInstruction &p,const char *pipeline) {
    ...
    if(store_op(p.instruction.op)){
      ...
      for(unsigned lane=0;lane<c.lanes;++lane)std::memcpy(spm.data()+c.arena*64+a_base+(row*16+lane)*out_bytes,&p.value.data[lane],out_bytes);
  ...
  void invariant() {
    ...
    for(unsigned p=0;p<pes;++p){unsigned count=0;for(unsigned s=0;s<options.contexts;++s){auto &c=contexts[p*2+s];if(!c.active)continue;++count;++live;
      for(unsigned v=c.arena;v<c.arena+arena_vectors;++v)check(spm_owner[v]==int(p*2+s),"live context lost its SPM arena");}
      check(count*6<=16,"live RF allocations exceed PE capacity");}
    for(int id:spm_owner)if(id>=0){++used;check(contexts[id].active,"retired context still owns SPM");}
  ...
  bool tick() {
    ...
    bool port=cycles%options.spm_period==0;
    std::array<bool,16> write_port{};write_port.fill(cycles%options.writeback_period==0);
    ...
    if(spm_pending&&spm_pending->due<=cycles){auto &c=validate(spm_pending->owner);
      if(store_op(spm_pending->instruction.op)){if(port){port=false;finish_spm=true;}}
      else if(write_port[c.pe]){write_port[c.pe]=false;finish_spm=true;}else ++writeback_stalls;}
    const auto incoming=memory_port->response();
    ...
    if(incoming){check(bool(dma),"unsolicited matrix memory response");check(incoming->id==dma->id,"stale matrix memory response token");check(!incoming->error,"matrix memory response reported error");validate(dma->owner);held_response=incoming;
      if(cycles%options.dma_response_period==0){if(dma->write)finish_dma=true;else if(port){port=false;finish_dma=true;}}}
    for(unsigned p=0;p<pes;++p)if(fu[p]&&fu[p]->due<=cycles){validate(fu[p]->owner);if(write_port[p]){write_port[p]=false;finish_fu[p]=true;}else ++writeback_stalls;}
    ...
    for(unsigned id:ids){auto &c=contexts[id];if(!eligible(id)||!instruction_phase(c)||c.busy||selected[c.pe])continue;
      const auto &i=instructions(c).at(c.pc);
      if(i.row<2&&i.row>=c.rows){selected[c.pe]=true;predicated.push_back(id);continue;}
      if(memory_op(i.op)){if(spm_pending||new_spm||!port)continue;port=false;new_spm=true;}
      else if(fu[c.pe]||cycles<next_compute[c.pe])continue;
      selected[c.pe]=true;issues.push_back(PendingInstruction{owner(id),i,evaluate(c,i),cycles+latency(i.op)});
    }
    std::optional<PendingDMA> new_dma;
    if(!dma&&cycles%options.dma_request_period==0&&memory_port->request_ready()){for(unsigned id:ids){auto &c=contexts[id];if(!eligible(id)||instruction_phase(c)||c.memory_busy)continue;
      if(c.phase==Drain&&!port)continue;
      new_dma=request(id);if(c.phase==Drain)port=false;break;}}
    ...
    if(finish_spm){complete_instruction(*spm_pending,"spm");spm_pending.reset();}
    if(finish_dma){auto pending=*dma;auto &c=validate(pending.owner);check(c.memory_busy,"DMA response has no request owner");
      ...
      else{pending.data=incoming->data;std::memcpy(spm.data()+pending.spm,&pending.data,pending.bytes);dma_read_bytes+=pending.bytes;numeric.memory_read_bytes+=pending.bytes;}
      ++dma_responses;dma_event("dma_response",pending);dma.reset();memory_port->consume_response();held_response.reset();finish_memory(pending.owner.context);
    }
    for(unsigned p=0;p<pes;++p)if(finish_fu[p]){complete_instruction(*fu[p],"compute");fu[p].reset();}
    for(unsigned id:predicated){auto &c=contexts[id];auto i=instructions(c).at(c.pc);++numeric.instructions;++numeric.opcode_counts[i.op];++numeric.inactive_row_instructions;event("predicated_issue",id,"control",i.op);finish_instruction(id);}
    ...
    if(admission){auto [id,start]=*admission;auto &c=contexts[id];uint64_t epoch=c.epoch+1;c=Context{};c.active=true;c.pe=id/2;c.slot=id%2;c.epoch=epoch;c.block=next_block++;
      c.row_base=(c.block/((n+15)/16))*2;c.col_base=(c.block%((n+15)/16))*16;c.rows=unsigned(std::min<uint64_t>(2,m-c.row_base));c.lanes=unsigned(std::min<uint64_t>(16,n-c.col_base));c.arena=start;
      for(unsigned v=start;v<start+arena_vectors;++v){check(spm_owner[v]<0,"non-atomic SPM admission");spm_owner[v]=int(id);}
      for(unsigned r=0;r<6;++r)reg(c,r).type=0;
  ...
  Json::Value result() const {
    ...
    unsigned active=0,allocated=0;for(const auto &c:contexts)active+=c.active;for(int id:spm_owner)allocated+=id>=0;
...
Simulator::Simulator(const Json::Value &p,Tensor a,Tensor b,const Tensor *bias,bool tb,uint64_t ab,uint64_t bb,
                     uint64_t m,uint64_t n,uint64_t k,Tensor out,uint64_t ob,Options o,model_io::MemoryPort *port)
    :impl(std::make_unique<Impl>(p,std::move(a),std::move(b),bias,tb,ab,bb,m,n,k,std::move(out),ob,o,port)){}
