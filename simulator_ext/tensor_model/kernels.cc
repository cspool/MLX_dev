...
Tensor Kernels::memory(const Json::Value &node,const Values &values) {
  ...
  memory_stats->split_view_outputs+=count.split_view_outputs;
...
Tensor Kernels::execute(const Json::Value &node,const Values &values) {
  ...
  if (kind=="linear" || kind=="matmul") out=matrix(node,values,kind=="linear");
    ...
    const std::vector<std::string> legal={"div","maximum","exp","sub","add","mul","le","where","pow","rsqrt","silu","cos","sin","neg"};
    ...
    for (uint64_t i=0;i<out.numel();++i) {
      ...
      else if (kind=="div") value=a/get(args[1],values,i,declared);
      else if (kind=="maximum") {float b=get(args[1],values,i,declared);value=a<b?b:a;}
      else if (kind=="exp") value=std::exp(a);
