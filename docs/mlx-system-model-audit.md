...
后续 [v2 大地址空间验证](mlx-paired-wide-system.md)已完成五个 16 GiB/4×4 登记图的独立审计。参考 token 检查按 logits 最后一维逐行执行，可核对广播矩阵等多行输出；完整 Llama2 的额外身份和完整性约束保持不变。完整镜像准备审计也已加入 v2 参数及完整 CPU ELF/检查器的逐字节重建，但准备与初始化不等于模型执行。
