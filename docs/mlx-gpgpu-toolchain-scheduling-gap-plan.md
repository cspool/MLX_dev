...
## 当前优先级补充（2026-09-09，用户明确）

### 当前执行优先级（用户最新要求，覆盖此前支线安排）

五项主线的已有证据、独立退出条件及当前 Chipyard 推进内容见[主线验收清单](mlx-mainline-acceptance.md)。

最新完整事件接入：[窗口目录链接器](mlx-full-event-linker.md)已将 BERT 全部 3,046 源、3,838 窗口及 693 配对连接为 C++ 紧凑源流；10 个实际数值小图的图周期对齐、7 项拒绝检查通过。完整链接计数不是整模执行结果，仍待独立整图审计、完整运行及最终读回边界对齐，不改变 Chipyard 整模优先级。
...
| P1：其次 | GPU实机、RTX4090/RTX5090等GPU模拟、校准与相关实验 | 暂停新增实验/扩展，保留已完成数据；待P0主线验收后继续 |
| P2：最后 | 对应RTL、RTL对照及PPA | 优先级低于GPU模拟和实机实验，当前不推进 |

已启动的独占完整MLX/Chipyard尝试继续追踪，保持来源、程序和二进制不变。当前两个Rocket尝试虽在推进，但仍处第0层矩阵阶段，因此工作重点包括系统路径的实际运行开销与验收缺口，而不是仅等待或转去扩展GPU支线。需要修改时使用独立构建和等价验证，不能缩减模型、用参考中间值替换计算，或把功能快路径冒充完整时序系统通过。

事件驱动路径增量：[原生C++并发事件核心](mlx-concurrent-event-core.md)已实现有限RF/SPM/ROM准入、模板共享/编程、事件日历、共享端口和内部使用计数；完整Llama2/BERT资源清单已提取。尚缺完整微程序与memory/control事件lowering，因此不报告模型级事件周期或端到端误差；不将组件检查替代完整模型目标。

循环基础继续补齐：[嵌套循环 IR](mlx-event-loop-ir.md)按需遍历全部动态实例，55 项回归及 58 次安全重放通过，和完全展开的并发流逐项一致；真实微程序/端口规则对齐和整模接入仍未完成，不提供整模预测值。

随后[端口契约与完整矩阵窗口](mlx-event-port-contract.md)已接入独立 DMA 发射、SPM/RF 写回竞争、谓词、准入节拍与矩阵微程序循环；22 个完整窗口与原生数值执行的周期及计数一致，95 项回归和 94 次安全重放通过。仍不是完整模型事件运行，跨源/其他后端接入及整模误差待完成。

向量路径继续补齐：[完整向量窗口事件 lowering](mlx-vector-event-lowering.md)覆盖逐元素、mean/softmax、多遍搬运和归约栈；48 个窗口与原生数据执行一致，原矩阵对照保留，148 项回归及 142 次安全重放通过。独立搬运、控制与整图跨源接入仍未完成，不能报告模型级性能误差。

独立搬运继续补齐：[C++ 搬运控制器事件路径](mlx-memory-event-lowering.md)保留独立寄存器/staging、真实索引/谓词工作与共享 DMA 竞争；18 个窗口对齐原生固定延迟执行，176 项回归及 166 次安全重放通过，矩阵/向量重放保留。完整控制流、跨源顺序、逐实例块间事件与整模接入仍未完成，暂不报告模型级误差。

控制窗口继续补齐：[描述符绑定 RV64 控制事件路径](mlx-control-event-lowering.md)覆盖 9 类入口的所有叶指令、循环、argmax 分支与 guard 成功路径约束，保留独立控制资源与共享总线竞争。28 个数值窗口与事件路径对齐，累计 221 项回归、202 次安全重放通过；完整源图、动态见证、跨源时序和整模误差仍待完成，不将该前端误称为 Rocket CPU 模拟。

跨源次序继续补齐：[按源推进与真实并发窗口组](mlx-event-source-order.md)对齐原生每源完成检查／发射顺序，并按原生模板 key 共享 ROM。32 个双源矩阵／向量组的周期、块生命周期和资源峰值对齐；旧全局完成优先在其中 9 组有差异，最大绝对相对误差约 2.63%。累计 261 项回归、272 次安全重放通过。这是窗口组误差，不是整模误差；完整图、batch、流式块依赖及控制／搬运混合仍待接入。

源图生命周期继续补齐：[源准入、依赖和批次发布](mlx-event-source-graph.md)明确前端槽在等待 PE 时仍占用、新源启动须共享 DMA 空闲、全部窗口完成后才发布。8 个真正消费前驱输出的六源依赖图对齐，累计 293 项回归和 291 次安全重放通过。完整 Llama2/BERT 已分别核对 12,133/3,838 个窗口的图清单，尚未将这些完整微程序按需装入并执行，不提供整模误差。

完整窗口编译继续补齐：[完整模型窗口与实际布尔见证](mlx-full-model-event-windows.md)通过完整 C++ BERT 只读重放取得真实 guard／where 值，最终输出与原生基线逐位一致。BERT 3,838 个窗口全部编译并重建审计；Llama2 编译 12,127/12,133 个，剩余六个窗口等待实际 where／argmax 数据。320 项回归通过，但尚未完整事件图执行，不能报告模型级时延或误差。

C++ 装入路径继续补齐：[块事件按需解析与退休回收](mlx-lazy-event-patterns.md)在同一调度器中按准入读取经摘要绑定的模式，退休后复用事件槽。342 项回归、324 次安全重放通过；BERT `[1,64,3072]` 完整尺寸窗口的原始／预展开／懒模式目标周期一致，懒执行槽峰值 350。完整块描述符仍预先保留，整模目录、流式块依赖和系统边界尚未接通，不作为整模性能误差结果。

宿主 ROM 记录也已加入退休复用，以独立 generation 保持原模板编程次序；2,000 次驻留仅需 1 个记录槽。344 项回归、328 次安全重放及完整尺寸窗口复查通过。此处不改变硬件 ROM 容量或时序，也未解决完整块描述符的预展开问题，整模验收仍待继续。

块实例生成继续补齐：[紧凑运行段与 C++ 窗口游标](mlx-compact-block-streams.md)保留源／batch／块身份及 PE 映射，只创建活动／待准入块，退休后删除对象，不再展开所有实例。369 项回归、364 次安全重放通过；BERT 完整尺寸窗口 12,288 块由 1 个运行段表示，描述符峰值 26，周期及工作不变。完整目录直接接入、流式块事件和系统边界仍待完成，不能据此报告整模误差。

流式配对继续补齐：[有限完成槽、生产者资源保留和注册访存](mlx-streaming-pair-events.md)复用原生 CompletionWindow，消费者可先驻留、按实际块可见性发射，矩阵 batch 保留完整编号。406 项回归、401 次安全重放和 25 个实际数值配对组对齐通过。比较边界仅为配对组；完整目录链接、实际地址／数据生命期和最终读回仍待完成，不作为整模误差或 Chipyard 通过。

架构比较及消融都保留并发：GPU保留SM/block/warp并发，MLX保留PE/上下文/流水线并发，研究并发利用率和调度效率。一次只消融具体机制，不把事件驱动等同于串行，也不把整个架构串行化作为正式基线。已有串行路径降为历史调试记录。RTX4090的GitHub实验适配和校准线索见[检索记录](rtx4090-simulator-search.md)。

新增交付（按用户最终澄清）：**MLX比较端到端执行模拟与事件驱动模拟的性能误差，两者都考虑并发和调度，以端到端执行为参照；GPU比较GPU模拟与RTX4090实测的误差，以实机为参照。** 不将事件驱动解释为串行累加，也不以串行／配对消融替代主实验。MLX/GPU跨架构速度比可另表报告。见[两组误差协议](mlx-gpu-performance-comparison.md)。已有[原生Llama2设备基线](mlx-native-inference-performance.md)保留为辅助证据；[完整BERT QA的C++微程序端到端结果](mlx-qa-result-runtime.md)已通过主要正确性，正在运行周期路径。

**优先完整端到端执行和主要正确性，容忍合理的 GPU 数值差异，尽快开始性能试验。** 不再将历史 GPU logits 容差失败本身作为所有性能工作的阻塞条件，也不要求与 GPU 逐位相同。检查重点为完整模型/参数/源算子实际执行、真实中间数据与 token/cache 反馈、最终任务结果、有限数值和已声明数值合约。实际 GPU 参考及原阈值/误差记录保留，不把失败记录改成通过。

已完成的公开 dense Llama2 原生串行路径，在全图/参数、96,000 logits 数值合约逐位一致和三个实际 token 一致的证据范围内，可以分析已记录周期作为**历史调试证据**，但按用户后续明确要求，不把该串行路径作为正式并发架构性能基线。GPU数值差异诊断保留。正式模型与消融均需保留各架构并发能力；真实系统性能仍须完整系统运行与主要正确性通过，不代表作者hybrid或其他模型已经通过。设备周期、宿主模拟耗时与实际CPU/装载开销分别报告，禁止缩小模型外推。后文此前的“原GPU门槛阻塞性能”是历史阶段状态，以本补充为准。实施顺序仍为C/C++模拟器 → 系统集成 → RTL。
...
完整运行终态（2026-09-08）：[公开dense Llama2原生串行C++执行](mlx-complete-native-llama2-result.md)已完成全部6,181个源调用，96,000个FP16 logits与预先绑定的数值合约逐字节一致，实际token/cache反馈及物理资源排空通过。但原GPU误差门槛仍有1,145/2,524/22个元素超差；不能因此宣布模型整体正确性、系统或性能通过。原生配对及两个Rocket完整尝试继续单独验收，不将该串行结果转移给其他实现或作者hybrid。
...
v2 完整模型前置条件继续补齐：[16 GiB/4×4 配对系统验证](mlx-paired-wide-system.md)的五个高地址登记图及独立系统审计全部通过，含观察开关等价检查。完整 Llama2 v2 镜像已重建 CPU ELF/检查器并在 C++ 内存对象中核对全部 360 资产，时钟/AXI计数为0；仅是准备与初始化，不是模型执行。151项相关检查通过，完整模型数值、性能及RTL门槛仍未关闭。

完整原生配对路径继续推进：[原生 C++ 模型入口与审计](mlx-native-paired-model-runner.md)已通过145项回归、15个安全重放，直接绑定实际 safetensors 参数范围及最终 C++ 输出。完整6181源/873对尝试已启动，尚待终态；控制 leaf 和宿主预装载仍与实际 Rocket/CPU/DMA 边界分开。两个原工作树中的既有运行不被修改或重标，所有完整模型正确性与性能门槛仍保持独立。

等待完整运行期间，在新分支进行[宿主准入查询缓存](mlx-host-admission-cache.md)优化：182项回归通过，10个登记图与旧二进制的数值、事件、周期和资源记录完全一致，并完成安全重放。此修改只减少未变化资源状态的重复扫描，不跳过模拟边沿、不扩大硬件资源；组件宿主耗时不是模型推理性能，四个长运行仍保留冻结版本。

模型数值差异继续排查：[独立整数精确投影诊断](mlx-exact-projection-diagnosis.md)确认首层完整Q/K/V微程序符合逐K FP32合约，并以同输入GPU选库/split-K对照排除本次设置变化能够消除该差异的假设。精确参考及安全重放通过，但原GPU端到端门槛、后续层与完整系统结果仍未通过，不改变性能/RTL顺序。

后续模型前置材料见[实际checkpoint与入口缺口](mlx-next-model-materials.md)：InternLM2实际BF16权重尚无对应dtype路径，BERT结构化checkpoint需恢复匹配构造，QA输出也不能套用Llama2检查器。只读材料与13项检查器测试不算模型加载、算子覆盖或推理通过；所有后续模型仍遵守完整身份、全算子路由和端到端退出条件。

对完整图的宿主开销定位后，新增[生命周期触发的缓冲清理](mlx-host-buffer-collection.md)：196项回归、13个旧/新完整图及安全重放一致，完整6181节点图的受限窗口日志也一致。该窗口按限额退出1，没有完整模型结果；修改不跳过周期，不改变四个冻结尝试，也不作为模型推理性能证明。

其他模型前端继续补齐：[完整 BERT QA baseline 参考](mlx-bert-qa-reference.md)严格加载全部108,893,186个F32参数，正常/padding/上下文扰动三组的双输出插桩位模式等价检查通过，38项相关测试通过。实际1,141次调用中仍有11种入口明确缺失，并发现mask触发的真实Boolean分支；下一步需要完整C++算子/控制与多输出路由，不能用采集常量回放分支，也不把参考执行当作MLX或作者hybrid通过。
