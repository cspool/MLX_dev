...
新增交付：比较完整端到端调度与实际串行事件执行的性能差异，并单列并发事件时长累加值，避免重复计入重叠；同模型/输入/权重与数值输出先对齐，完整终态后才计算周期比。当前[原生Llama2设备基线及对照口径](mlx-native-inference-performance.md)已生成；[完整BERT QA的C++微程序端到端结果](mlx-qa-result-runtime.md)已通过主要正确性，正在运行其串行与配对周期对照。
