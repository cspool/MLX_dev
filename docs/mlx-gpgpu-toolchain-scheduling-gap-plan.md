...
新增交付（按用户随后澄清）：内部对照是完整端到端调度模拟与**事件延迟累加估计**，事件累加不是另一种完整数据执行；外部主对照是MLX端到端模拟按明确频率换算的时间与真实GPU实测时间。另评估GPGPU-Sim/Accel-Sim与相同GPU架构模型，须先校准其对实卡的误差。已有实际串行完整运行保留为辅助证据，不能替代以上主对照。见[GPU实测、事件估计与GPU模拟对照协议](mlx-gpu-performance-comparison.md)。当前[原生Llama2设备基线](mlx-native-inference-performance.md)已生成；[完整BERT QA的C++微程序端到端结果](mlx-qa-result-runtime.md)已通过主要正确性，正在运行周期对照。
...
已完成的公开 dense Llama2 原生串行路径，在全图/参数、96,000 logits 数值合约逐位一致和三个实际 token 一致的证据范围内，可开始**原生设备侧性能基线试验**，同时保留 GPU 差异诊断。此授权不代表作者 hybrid、其他模型或 Rocket 系统已经通过；真实系统的端到端性能仍须完整系统运行与主要正确性通过。设备周期、宿主模拟耗时与实际 CPU/装载开销分别报告，禁止缩小模型外推。后文此前的“原 GPU 门槛阻塞性能”是历史阶段状态，以本补充为准。实施顺序仍为 C/C++ 模拟器 → 系统集成 → RTL。
