...
2026-09-09用户更新验收优先级：先跑通完整端到端和主要正确性，合理GPU数值误差可容忍，尽快开展性能试验。严格GPU数值比较保留为诊断，不再单独阻塞已通过完整执行、实际数据/任务输出和声明数值合约的路径。正式架构性能与消融保留并发，已有逐算子串行Llama2周期只作历史调试，不作正式基线；MLX性能误差比较保留并发的端到端与事件驱动模型，GPU性能误差比较模拟与4090实机。系统级性能仍需真实系统完整执行，不能将原生结果转移为Rocket或作者hybrid通过。未来模型的主要正确性条件在运行前绑定，出现非有限数值、任务结果变化、缺算子/参数或参考中间结果回填仍应失败。历史严格比较结果不重写；详见[主计划的当前优先级](mlx-gpgpu-toolchain-scheduling-gap-plan.md#当前优先级补充2026-09-09用户明确)。
