...
本页补充模型级 ME0/ME1 的实际前置条件，避免把“目录里有权重”当作“已能在 MLX 执行”。下面材料审计只读本地 checkpoint 元数据，不实例化模型、不执行自定义模型代码、不采集前向算子，不替换作者模型身份，也不改变四个正在运行的 Llama2 尝试。

后续已单独建立[完整 BERT QA baseline 参考入口](mlx-bert-qa-reference.md)：保留完整 F32 参数并严格迁移名称，正常/padding/上下文扰动三组均完成参考前向及插桩位模式等价检查，实际采集 1,141 次调用、30 种 overload。它只补齐这个 baseline 的参考/清单子项；作者结构化构造、全部 C++ 算子路由、系统输出与性能仍未通过。下面表格中的“直接接入障碍”保留为材料审计时的状态。
